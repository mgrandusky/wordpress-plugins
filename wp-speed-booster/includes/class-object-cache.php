...
class WPSB_Object_Cache {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_wpsb_install_dropin', array( $this, 'ajax_install_dropin' ) );
		add_action( 'wp_ajax_wpsb_remove_dropin', array( $this, 'ajax_remove_dropin' ) );
		add_action( 'wp_ajax_wpsb_get_cache_stats', array( $this, 'ajax_get_stats' ) );
	}
	...
	public function generate_dropin() {
		...
		if ( ! $result ) {
			return new WP_Error( 'write_failed', __( 'Failed to write drop-in file', 'wp-speed-booster' ) );
		}
	...
	private function generate_dropin_content( $backend, $options ) {
		switch ( $backend ) {
			...
			default:
				return new WP_Error( 'invalid_backend', __( 'Invalid cache backend', 'wp-speed-booster' ) );
		}
	}
...
PHP;
	...
	public function ajax_install_dropin() {
		...
		if ( is_wp_error( $result ) ) {
			...
			wp_send_json_success( array( 'message' => __( 'Object cache drop-in installed successfully', 'wp-speed-booster' ) ) );
		}
	}
	...
	public function ajax_remove_dropin() {
		...
		if ( $result ) {
			wp_send_json_success( array( 'message' => __( 'Object cache drop-in removed successfully', 'wp-speed-booster' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to remove drop-in', 'wp-speed-booster' ) ) );
		}
	}
